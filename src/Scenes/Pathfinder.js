class Pathfinder extends Phaser.Scene {
    ...
    create() {
        ...
        window.tinyTownGrid = this.layersToGrid([this.groundLayer, this.treesLayer, this.housesLayer]);
    ...
    layersToGrid() {
        ...
        //2d grid: loop through each row, and add a list the size of the map width
        for(let i = 0; i < this.map.height; i++){
            grid[i] = new Array(this.map.width);
        }
        ...
        for(let layer of arrayOfLayers){
            ...
            for(let y = 0; y < this.map.height; y++){
                ...
                for(let x = 0; x < this.map.width; x++){
                    //get tile and if it isn't null, assign it to the grid
